...
class GcsPurchase(models.Model):
    ...
    def gcs_popup(self):
        purchase_report_template_id = self.env.ref('purchase.action_report_purchase_order')._render_qweb_pdf(self.id)
        purchase_order_ref = self.name + '.pdf'
        attachments = []
        data_record = base64.b64encode(purchase_report_template_id[0])
        ir_values = {
            'name': purchase_order_ref,
            'type': 'binary',
            'datas': data_record,
            'store_fname': 'Purchase Order Reports.pdf',
